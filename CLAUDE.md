...
Manual gate — not yet wired into CI (`.github/workflows/cic-governance-ci.yml` governs lineage/build, not this checklist):

1. Before merging changes that touch CodeFlow-analyzed code, run `cic roadmap drift <repo>` (see GOVERNANCE.md § Drift Detection) and resolve any `high`/`critical` severity before requesting review.
2. Before cutting a release, run `cic release gates` — release is blocked if any `must-adopt` + `blocking: true` roadmap item is not `active`.
3. After editing `policy-manifest.yaml`, run `cic roadmap policy --reload` and re-check drift; stale policy silently under-enforces.
4. Treat drift severity `high`+ as a `must-adopt` roadmap item per policy — do not manually downgrade without recording the reason via `cic roadmap show <itemId> --history`.
